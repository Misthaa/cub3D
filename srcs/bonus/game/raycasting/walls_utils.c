...
/*   Updated: 2024/06/10 04:06:27 by ehalliez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_wall_height(t_cub *cub, double *distances, int x)
{
	...
	corrected_distance = distances[x] * cos(cub->map->player_angle - \
					...
					(x * (FOV / cub->settings->width))));
	distance_to_projection_plane = (cub->settings->width / 2) / tan(FOV / 2);
